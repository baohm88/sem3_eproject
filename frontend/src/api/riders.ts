...
import type { RiderProfile, PageResult, Wallet, Transaction, Order, Review } from './types';
...
export async function topup(userId: string, payload: { amountCents: number; idempotencyKey?: string }) {
...
export async function withdraw(userId: string, payload: { amountCents: number; idempotencyKey?: string }) {
  const res = await api.post(`/api/riders/${userId}/wallet/withdraw`, payload);
  return res.data.data;
}

export async function createOrder(userId: string, payload: { companyId: string; serviceId: string }) {
  const res = await api.post(`/api/riders/${userId}/orders`, payload);
  return res.data.data as Order;
}

export async function listOrders(userId: string, params: Record<string, any> = {}) {
  const res = await api.get(`/api/riders/${userId}/orders`, { params });
  return res.data.data as PageResult<Order>;
}

export async function cancelOrder(userId: string, orderId: string) {
  const res = await api.post(`/api/riders/${userId}/orders/${orderId}/cancel`);
  return res.data.data as Order;
}

export async function reviewOrder(userId: string, orderId: string, payload: { rating: number; comment?: string }) {
  const res = await api.post(`/api/riders/${userId}/orders/${orderId}/reviews`, payload);
  return res.data.data as Review;
}

export async function listMyReviews(userId: string, params: Record<string, any> = {}) {
