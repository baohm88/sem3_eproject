// src/pages/Drivers/DriverJobsPage.jsx
...
export default function DriverJobsPage() {
  ...
  // Fetch driver's employment status
  useEffect(() => {
  ...
  // Load applications of this driver to mark already-applied companies
  useEffect(() => {
  ...
  // Debounce filter changes -> refetch page 1
  useEffect(() => {
  ...
  const doRecallApplication = async () => {
    ...
    try {
      ...
      if (!appId) {
        toast.error("Could not find an application to cancel");
        return;
      }
      await cancelApplication(driverUserId, appId); // <-- new DELETE endpoint
      toast.success(`Canceled application at ${recallingCompany.name}`);
    ...
    <div>
      ...
      <ConfirmModal
        ...
        message={
          applyingCompany ? (
            <div>
              Apply to <strong>{applyingCompany.name}</strong>?
              <br />
      ...
      <ConfirmModal
        ...
        message={
          recallingCompany ? (
            <div>
              Are you sure you want to cancel your application at{" "}
