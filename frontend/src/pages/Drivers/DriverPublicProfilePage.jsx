...
// ⭐ Star rating display (0..5 with halves)
...
export default function DriverPublicProfilePage() {
  ...
  // Normalize skills once; supports string JSON or CSV from API
  const skills = useMemo(() => jsonOrCsvToArray(driver?.skills), [driver?.skills]);

  // Initial fetch of the public profile
  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
  ...
  if (!driver) {
    return <div className="py-5 text-center text-muted">Driver not found.</div>;
  }

  // Prefer driver image; fall back to placeholder
  const imgSrc = driver.imgUrl && driver.imgUrl.trim() ? driver.imgUrl : FALLBACK_AVATAR;
        ...
        <Col xs={12} md={4} lg={3}>
          <Card className="shadow-sm">
            <Card.Body className="text-center">
              ...
              <div className="text-muted small">{driver.location || "—"}</div>
              <div className="mt-2">
                <Stars value={data?.rating ?? driver.rating ?? 0} />
              </div>
              <div className="mt-2">
                {driver.isAvailable ? (
                ...
                <div className="d-flex align-items-center gap-3">
                  ...
                  <div>
                    <div className="fw-semibold">
                      <Link to={`/companies/${data.hiredCompany.id}`}>
                        {data.hiredCompany.name}
                      </Link>
                    </div>
                    <div className="text-muted small">
                      Plan: {data.hiredCompany.membership} • Rating:{" "}
                      {Number(data.hiredCompany.rating ?? 0).toFixed(1)}
                    </div>
                  </div>
                </div>
              ) : (
        ...
        <Col xs={12} md={8} lg={9}>
          <Row className="g-3">
            ...
            <Col xs={12}>
              <Card className="shadow-sm">
                <Card.Body>
                  <Card.Title className="h6">About</Card.Title>
                  <div className="text-muted">{driver.bio || "—"}</div>
                </Card.Body>
              </Card>
            </Col>
            ...
            <Col xs={12}>
              <Card className="shadow-sm">
                <Card.Body>
                  <Card.Title className="h6">Skills</Card.Title>
                  {skills.length ? (
            ...
            <Col xs={12}>
              <Card className="shadow-sm">
                <Card.Body>
                  <Card.Title className="h6">Employment History</Card.Title>
                      ...
                      {data.employmentHistory.map((e) => (
                        ...
                        >
                          ...
                          <div>
                            ...
                            <div className="text-muted small">
                              Since {new Date(e.since).toLocaleDateString()} • Base salary:{" "}
                              {/* NOTE: API value appears to be in cents; rendered raw per current UI */}
                              {e.baseSalaryCents}₫
                            </div>
                          </div>
                        </div>
            ...
            <Col xs={12}>
              <Card className="shadow-sm">
                <Card.Body>
                  <Card.Title className="h6">Ratings & Comments</Card.Title>
                  <div className="text-muted">—</div>
                </Card.Body>
              </Card>
            </Col>
